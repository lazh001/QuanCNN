...
def run(cfg: DictConfig):
    local_rank = int(os.environ.get('LOCAL_RANK', 0))                                   # local_rank指的是multiprocessing的rank，rank==0意味着是程序主支
    ...
    existing_ver = list()                                                               # Ver是Version
    for d in experiment_dir.iterdir():
        if d.name.startswith('v') and d.name[1:].isdecimal() and d.is_dir():            # d.name[1:]是对name这个字符串的切片
    ...
    log_file_path = output_dir / f"run_rank{local_rank}.log"
    print(f'Rank {local_rank} log file: {log_file_path}')
    logging.config.fileConfig(script_dir / 'logging.conf', disable_existing_loggers=False,
                              defaults={'log_filename': str(log_file_path).replace('\\', '/')})
    ...
    if cfg.quan is not None:
        ...
        pl_logger.info('No quantizer is injected into the model.')
    ...
    if cfg.eval:
        ...
        trainer = pl.Trainer(logger=tensorboard,
                             ...
                             **cfg.trainer)
        pl_logger.info(f'The model is distributed to {trainer.num_devices} GPU nodes')
...
if __name__ == '__main__':
    conf = apputil.get_config(base_conf_filepath=Path.cwd() / 'conf' / 'cnn' / 'quanCnn.yaml')
    run(cfg=conf)
